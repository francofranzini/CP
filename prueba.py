...
for obj in self:
    ...
    if obj.stage_id.codigo == 'PROG' and aplicaProgramacion(obj):
        ...
        #Fechas desde/hasta vacias en personal Contratado:
            if obj.env['x_tarea_contratados'].search([('x_tarea_id', '=', obj.id), ('x_fecha_hasta', '!=', None)]):
                ...
                validacion = True
            if obj.env['x_tarea_contratados'].search([('x_tarea_id', '=', obj.id), ('x_fecha_desde', '!=', None)]) and esProgramada(obj):
                mensaje += 'El personal contratado NO debe tener fecha desde \n'
                validacion = True
            if obj.env['x_tarea_contratados'].search([('x_tarea_id', '=', obj.id), ('x_fecha_desde', '=', None)]) and esNoProgramada(obj):
    ...
    if obj.stage_id.codigo == 'FIN' and obj.categoria_id.x_aplica_programacion:
        ...
        sinFinalizadas = 0
        for tarea in obj.child_ids:
            if tarea.stage_id.codigo != 'FIN':
                mensaje += "Deben estar finalizadas las tareas hijas para proceder"
                validacion = True
                break
if validacion:
